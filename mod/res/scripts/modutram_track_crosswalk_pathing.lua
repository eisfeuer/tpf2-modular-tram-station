...
local function get_crosswalk_y_position(self, direction, segment_field)
    if self.left_platform == nil and self.right_platform == nil then
        return nil
    end

    if self.left_platform == nil then
        return self.right_platform[segment_field]
    end

    if self.right_platform == nil then
        return self.left_platform[segment_field]
    end

    return math.max(self.left_platform[segment_field] * direction, self.right_platform[segment_field] * direction) * direction
end
...
function TrackCrosswalkPathing:get_bottom_crosswalk_y_position()
    return get_crosswalk_y_position(self, -1, 'btm_segment_id')
end

function TrackCrosswalkPathing:get_top_crosswalk_y_position()
    return get_crosswalk_y_position(self, 1, 'top_segment_id')
end
...
function TrackCrosswalkPathing:add_top_crosswalk_to_model_collection(model_collection, model)
    local crosswalk_position = self:get_top_crosswalk_y_position()
    if crosswalk_position ~= nil then
        model_collection:add({
            id = model,
            transf = Position:new{x = self.track.x_pos, y = (crosswalk_position + 1.5) * c.PLATFORM_SEGMENT_LENGTH}:as_matrix()
...
function TrackCrosswalkPathing:add_bottom_left_vertical_pathes_to_model_collection(model_collection, model, last_crosswalk_position)
    if self.left_platform then
        local crosswalk_position = get_crosswalk_y_position(self, -1, 'btm_segment_id')
...
function TrackCrosswalkPathing:add_top_left_vertical_pathes_to_model_collection(model_collection, model, last_crosswalk_position)
    if self.left_platform then
        local crosswalk_position = get_crosswalk_y_position(self, 1, 'top_segment_id')
        if has_vertical_path_models(self.left_platform.top_segment_id, crosswalk_position, last_crosswalk_position, 1) then
            local from, to = get_vertical_path_placing_range(self.left_platform.top_segment_id, crosswalk_position, last_crosswalk_position, 1)
            for i = from + 1, to, 1 do
                model_collection:add({
                    id = model,
                    transf = Position:new{x = self.left_platform.x_pos, y = (i + 1) * c.PLATFORM_SEGMENT_LENGTH}:as_matrix()
